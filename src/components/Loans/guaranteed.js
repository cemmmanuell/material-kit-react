...
export default function GuaranteedLoans() {
...
useEffect(() => {
  (async () => {
    setUserLoaded(false);
    let res = await ReactSession.get('user_details').data.guaranteeDetails;
    
      SetRows(res.filter(r=>r.type=="Guaranteed"));
      setUserLoaded(true);
...
async function  fechdata(){
  ...
  return (
     ...
     <Card>
      <CardHeader title="My guaranteed loans" />
      <CardContent> 
     <Grid xs={12} sm={12}>
